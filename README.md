...
### 関係者に全部の状態を見てもらう（プレビュー）

URL の末尾に `?preview=...` を付けると、**その状態の画面を実機で確認できます。**
画面上部に「確認用の表示です。実際の販売状況ではありません。」というバーが必ず出て、
そこから5つの状態をタップで切り替えられます。**リンク1本送れば全部見てもらえます。**

| URL | 見えるもの |
|---|---|
| `?preview=waiting` | 通常の待機中 |
| `?preview=imminent` | 販売10分前（カウントダウン拡大） |
| `?preview=final` | 販売1分前（秒を強調・約40秒後に販売開始へ切り替わります） |
| `?preview=onsale` | 販売中（Amazon CTA） |
| `?preview=soldout` | 販売終了 |
| `?preview=live&sec=90` | 90秒後に販売開始（切り替わる瞬間を見せたいとき） |

- **プレビュー中は計測を一切送りません。** GA4 タグも読み込まないので、
  確認のための閲覧が実際のファネルの数字を汚すことはありません。
- `?preview=` が付いていない通常の閲覧には**一切影響しません**。
