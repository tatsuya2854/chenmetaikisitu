...
| 完売したら | `saleStatus` を `'sold_out'` に変更。**ページを開いたままの人にも30〜40秒ほどで届きます**（リロード不要） |
...
### 設定変更が「開いたままの人」に届く仕組み

ページは開いている間、`config.js` を定期的に読み直しています。
そのため、**売り切れて `saleStatus` を `'sold_out'` にすると、
すでにページを開いている人の画面もリロードなしで切り替わります。**

| 状況 | 確認の間隔 |
|---|---|
| 販売中 | 約30秒ごと（22〜37秒） |
| それ以外 | 約5分ごと |
| タブが裏にあるとき | 確認しない（戻ってきたら即確認） |
| プレビュー表示中 | 確認しない |

間隔を毎回 ±25% ずらしているのは、**販売開始の瞬間に全員が同時に
設定を取りに行かないようにするため**です（アクセスが一点に集中するのを避ける）。

**即時に反映される項目**: 販売ステータス / 販売日時 / Amazon URL
（販売中にURLの間違いに気づいても、その場で直せます）

**リロードが必要な項目**: 文言、通知設定、販売実績、GA4測定ID
